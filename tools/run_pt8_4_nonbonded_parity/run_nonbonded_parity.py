...
def create_gro_from_lammps(lammps_data_path: Path, gro_path: Path):
    ...
    box_x = 10.0 # Large box to avoid PBC issues if not intended
    box_y = 10.0
    box_z = 10.0
...
def check_system(system_id: str, corpus_root: Path, results_root: Path):
    ...
    print(f"--- Running nonbonded parity check for {system_id} ---")
    ...
    with (work_dir / "nonbonded_parity_report.json").open("w") as f:
        json.dump(report, f, indent=2)
    return report

def main():
    systems = ["lj96_toy", "exclusion_toy"]
    corpus_root = REPO_ROOT / "testdata" / "lammps_golden"
    results_root = REPO_ROOT / "tests" / "reference_results" / "pt8_4_nonbonded_parity"
    ...
    with (results_root / "nonbonded_parity_summary.json").open("w") as f:
