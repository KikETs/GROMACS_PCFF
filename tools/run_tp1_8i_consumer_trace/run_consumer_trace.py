...
RESULTS_DIR = ROOT / "tests/reference_results/tp1_8i_consumer_trace"

TP13_DIR = ROOT / "tests/reference_results/tp1_3_stabilization/TRL-0"
TP18H_RESULTS = ROOT / "tests/reference_results/tp1_8h_update_trace"
...
RUNS = [
    {
        ...
        "fourierspacing": 0.12,
        "ewald_rtol": 1e-5,
        "why": "Reuses the TP1.8h authoritative safe baseline settings for kinetic/temperature consumer tracing.",
    ...
    },
    {
        ...
        "fourierspacing": 0.12,
        "ewald_rtol": 1e-5,
        "why": "Switches the Coulomb solver from PME to Ewald while keeping the same authoritative short-range baseline and active kinetic consumer path.",
        "intended_path_change": "pme_vs_ewald_kinetic_temperature_consumer_trace",
    },
]

TRACE_STAGE_FIELDS = ["stage"]
TRACE_INT_FIELDS = [
    "b_gstat",
    "b_energy",
    "b_temperature",
...
TRACE_FLOAT_FIELDS = [
    ...
    "tcstat_ekinh_old_l2_sum",
    "tcstat_ekinf_trace_sum",
    "tcstat_ekinf_l2_sum",
    "tcstat_temperature_sum",
    "tcstat_temperature_max",
    "tcstat_halfstep_temperature_sum",
    "tcstat_halfstep_temperature_max",
    "ekind_ekin_trace",
    "ekind_ekin_l2",
    "dekindl",
    "dekindl_old",
...
TP18H_BASELINE_MDOUT = TP18H_RESULTS / "raw_safe_pme_shift_ref_mdout.mdp"
REUSE_FIELDS = [
    "integrator",
    "dt",
    "nsteps",
...
def compare_stage_rows(reference_rows: list[dict[str, object]], variant_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    early_pairs = [
        ...
        if int(ref_row["step"]) <= EARLY_STEP_MAX
    ]
    summary: dict[str, object] = {
...
def run_case(run: dict[str, object], commands: list[str]) -> dict[str, object]:
    ...
    env = {"GMX_TP18I_TRACE_FILE": str(trace_output)}
    ...
    if not trace_output.exists():
        raise RuntimeError(f"{run_id} did not produce TP1.8i trace output")
...
def build_path_map(summary: dict[str, object]) -> dict[str, object]:
    baseline_stage_summary = summary["baseline"]["trace_summary"]["stages"]
    after_gstat = baseline_stage_summary.get("after_gstat_block", {})
    after_sum_ekin = baseline_stage_summary.get("after_sum_ekin", {})
    return {
        "milestone": "TP1.8i",
        ...
        "paths": [
            {
                ...
                "role": "Calls compute_globals immediately after update_coords on the active authoritative path.",
                "status": "active",
                "why": "TP1.8h already showed the PME-vs-Ewald force-side split survives through the update boundary and reaches compute_globals.",
            },
            {
                "file": "src/gromacs/mdlib/md_support.cpp",
                "function": "compute_globals",
                "role": "Immediate post-update kinetic/temperature consumer boundary.",
                "status": "active_and_traced",
                "why": "TP1.8i instruments the active simulator-owned kinetic/temperature path here instead of using another proxy.",
            },
            {
                "file": "src/gromacs/mdlib/md_support.cpp",
                "function": "calc_ke_part / calc_ke_part_normal",
                "role": "Consumes post-update velocities and fills tcstat half-step/full-step kinetic tensors plus dekindl.",
                "status": "active_and_traced",
                "why": "This is the first active kinetic accumulation stage after Update::update_coords.",
            },
            {
                "file": "src/gromacs/mdlib/stat.cpp",
                "function": "global_stat",
                "role": "Optional rank reduction of kinetic tensors before temperature aggregation.",
                "status": "inactive_effectively_single_rank"
                if not after_gstat.get("mpi_parallel_any", False)
                else "active",
                "why": "The authoritative reruns use a single-rank execution path, so no MPI reduction explains the observed split here.",
            },
            {
                "file": "src/gromacs/mdlib/tgroup.cpp",
                "function": "sum_ekin",
                "role": "Aggregates tcstat data into system temperature and total kinetic energy.",
                "status": "active_and_traced" if after_sum_ekin else "uncertain",
                "why": "This is the next simulator-owned temperature/kinetic consumer after calc_ke_part on the active path.",
            },
        ],
    }
...
def main() -> None:
    ...
    summary = {
        ...
        "baseline_reuses_tp18h_safe_settings": all(
            compare_mdout_fields(RESULTS_DIR / "raw_safe_pme_shift_ref_mdout.mdp", TP18H_BASELINE_MDOUT, REUSE_FIELDS).values()
        ),
        "baseline_vs_tp18h_field_match": compare_mdout_fields(
            RESULTS_DIR / "raw_safe_pme_shift_ref_mdout.mdp", TP18H_BASELINE_MDOUT, REUSE_FIELDS
        ),
        ...
        "interpretation": {
            "next_active_consumer_after_update_coords": "compute_globals -> calc_ke_part -> sum_ekin",
            "global_stat_rank_reduction_active": baseline["summary"]["trace_summary"]["stages"]
            .get("after_gstat_block", {})
            .get("gstat_reduction_executed_any", False),
            "simulator_owned_temperature_term_traced": "after_sum_ekin" in comparison["stages"],
            "simulator_owned_kinetic_energy_term_traced": "after_sum_ekin" in comparison["stages"],
        },
    ...
    recommendation = {
        ...
        "final_classification": "still_unresolved",
        "exact_next_step_recommendation": "If tracing continues at all, target only the next active kinetic/temperature consumer after compute_globals; otherwise stop at unresolved rather than claiming a localized root cause.",
    }
    ...
    run_matrix = {
        ...
        "tp18h_baseline_source": str(TP18H_RESULTS.relative_to(ROOT)),
        "reference_run_id": "safe_pme_shift_ref",
        "comparison_runs": RUNS,
        "rationale_for_20ps_window": "TP1.8h used the same 20 ps authoritative window with runaway onset at 0.2 ps, so TP1.8i preserves that same window while tracing the next active kinetic/temperature consumer.",
    }
    ...
    write_text(RESULTS_DIR / "consumer_path_map.json", json.dumps(build_path_map(summary), indent=2) + "\n")
    write_text(RESULTS_DIR / "tp1_8i_recommendation.json", json.dumps(recommendation, indent=2) + "\n")
    ...
    provenance = {
        ...
        "inputs": {
            ...
            "tp18h_results": str(TP18H_RESULTS.relative_to(ROOT)),
        },
    }
