...
RESULTS_DIR = ROOT / "tests/reference_results/tp1_8f_compute_globals_trace"
...
RUNS = [
    {
        ...
        "trace_filename": "compute_globals_trace_baseline.csv",
        "coulombtype": "PME",
        ...
        "fourierspacing": 0.12,
        "ewald_rtol": 1e-5,
        "why": "Reuses the TP1.8e authoritative safe baseline settings for compute_globals-level tracing.",
        "intended_path_change": "reference",
    },
    {
        "run_id": "safe_ewald_shift",
        "role": "ewald_variant",
        "trace_filename": "compute_globals_trace_variant.csv",
        "coulombtype": "Ewald",
        "coulomb_modifier": "Potential-shift-Verlet",
        "nstlist": 10,
        "rlist": 0.911,
        ...
        "intended_path_change": "pme_vs_ewald_compute_globals_trace",
    },
]
...
TRACE_FLOAT_FIELDS = [
    ...
    "shake_vir_trace_in",
    "shake_vir_l2_in",
    "total_vir_trace_before",
    "total_vir_l2_before",
    "pres_trace_before",
    "pres_l2_before",
    "total_vir_trace_after",
    "total_vir_l2_after",
    "pressure_fac",
    "pres_trace_after",
    "pres_l2_after",
...
def compare_traces(reference_rows: list[dict[str, object]], variant_rows: list[dict[str, object]]) -> dict[str, object]:
    ref = {int(row["call_index"]): row for row in reference_rows}
    var = {int(row["call_index"]): row for row in variant_rows}
    shared_call_indices = sorted(set(ref) & set(var))
    early_call_indices = [call_index for call_index in shared_call_indices if int(ref[call_index]["step"]) <= EARLY_STEP_MAX]

    comparison: dict[str, object] = {
        "shared_call_count": len(shared_call_indices),
        "early_shared_call_count": len(early_call_indices),
        "step_mismatch_count": sum(1 for call_index in shared_call_indices if int(ref[call_index]["step"]) != int(var[call_index]["step"])),
    }
    for field in TRACE_INT_FIELDS:
        comparison[f"{field}_mismatch_count"] = sum(
            1 for call_index in shared_call_indices if int(ref[call_index][field]) != int(var[call_index][field])
        )
    for field in TRACE_FLOAT_FIELDS:
        comparison[f"mean_abs_delta_{field}_full"] = mean(
            [abs(float(ref[call_index][field]) - float(var[call_index][field])) for call_index in shared_call_indices]
        )
        comparison[f"mean_abs_delta_{field}_early"] = mean(
            [abs(float(ref[call_index][field]) - float(var[call_index][field])) for call_index in early_call_indices]
        )
    return comparison
...
def run_case(run: dict[str, object], commands: list[str]) -> dict[str, object]:
    ...
    env = {"GMX_TP18F_TRACE_FILE": str(trace_output)}
    ...
    if not trace_output.exists():
        raise RuntimeError(f"{run_id} did not produce TP1.8f trace output")
        ...
        "paths": [
            {
                ...
                "why": "TP1.8f reuses the established PME-vs-Ewald split and traces only around compute_globals.",
            },
            {
                "file": "src/gromacs/mdrun/md.cpp",
                "function": "LegacySimulator::do_md",
                "role": "Caller-side handoff from TP1.8e that feeds compute_globals.",
                "status": "active_upstream_reference",
                "why": "TP1.8e already showed the force-side difference survives to the compute_globals call boundary.",
            },
            {
                "file": "src/gromacs/mdlib/md_support.cpp",
                "function": "compute_globals",
                "role": "Adds force_vir and shake_vir, updates total_vir, and writes the scalar pressure term.",
                "status": "active_and_traced",
                "why": "TP1.8f records pre/post virial and pressure summaries inside compute_globals.",
            },
            {
                "file": "src/gromacs/mdlib/coupling.cpp",
                "function": "calc_pres",
                "role": "Linear pressure tensor transform from ekin and virial using the current box volume.",
                "status": "active_formula_consumer_traced_at_caller_boundary",
                "why": "TP1.8f checks whether the compute_globals output follows the calc_pres formula or introduces extra divergence.",
            },
            {
                "file": "src/gromacs/mdlib/stat.cpp",
                "function": "global_stat",
                "role": "Optional reduction of energies and virials before compute_globals pressure handling.",
                "status": "inactive_effectively_single_rank",
                "why": "These TP1.8f reruns are single-rank, so no MPI reduction path is needed to explain the observed difference.",
            },
        ],
...
def main() -> None:
    ...
    summary = {
        ...
        "interpretation": {
            "tp1_4_lj_pme_path_status": "inactive",
            "per_component_virial_or_force_decomposition_available": False,
            "compute_globals_internal_trace": True,
        },
    ...
    recommendation = {
        ...
        "final_classification": "still_unresolved",
        "tp1_4_lj_pme_path_status": "inactive",
        "exact_next_step_recommendation": "Keep the authoritative safe baseline fixed and only trace deeper into compute_globals-adjacent pressure/virial consumers if the TP1.8f internal pressure handoff still leaves ambiguity; do not patch production logic yet.",
    }
    ...
    run_matrix = {
        ...
        "rationale_for_20ps_window": "TP1.8c through TP1.8e used the same 20 ps authoritative trace window with runaway onset at 0.2 ps, so TP1.8f keeps the same window while tracing inside compute_globals.",
    }

    write_text(RESULTS_DIR / "run_matrix.json", json.dumps(run_matrix, indent=2) + "\n")
    write_text(RESULTS_DIR / "compute_globals_path_map.json", json.dumps(build_path_map(), indent=2) + "\n")
    write_text(RESULTS_DIR / "compute_globals_trace_summary.json", json.dumps(summary, indent=2) + "\n")
    write_text(RESULTS_DIR / "tp1_8f_recommendation.json", json.dumps(recommendation, indent=2) + "\n")
