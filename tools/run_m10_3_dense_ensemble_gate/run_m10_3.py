...
def check_system(sid: str, corpus_root: Path, results_root: Path):
    ...
    print(f"--- M10.3 Dense Ensemble Parity Gate for {sid} ---")
    ...
    vol_lmp = lmp_stats["mean"][9] / 1000 # A3 to nm3
    ...
    report = {
        ...
        "parity_status": "pass" if dens_diff_rel < 0.05 else "fail"
    }
    ...
    print(f"  GMX Density: {dens_gmx:.2f} kg/m3 ({report['gmx']['density']['status']})")
    print(f"  LMP Density: {dens_lmp:.2f} kg/m3 ({report['lmp']['density']['status']})")
...
def main():
    ...
    results_root = REPO_ROOT / "tests" / "reference_results" / "m10_3_dense_ensemble_gate"
    results_root.mkdir(parents=True, exist_ok=True)
    
    results = []
    results.append(check_system("dense_oligomer", corpus_root, results_root))
    
    with (results_root / "m10_3_summary.json").open("w") as f:
