...
RESULTS_DIR = ROOT / "tests/reference_results/tp1_8d_coulomb_consumer_trace"
...
RUNS = [
    {
        ...
        "trace_filename": "downstream_trace_baseline.csv",
        "coulombtype": "PME",
        ...
        "fourierspacing": 0.12,
        "ewald_rtol": 1e-5,
        "why": "Reuses the TP1.8c authoritative safe baseline for downstream consumer tracing.",
        "intended_path_change": "reference",
    },
    {
        "run_id": "safe_ewald_shift",
        "role": "ewald_variant",
        "trace_filename": "downstream_trace_variant.csv",
        "coulombtype": "Ewald",
        "coulomb_modifier": "Potential-shift-Verlet",
        "nstlist": 10,
        "rlist": 0.911,
        ...
        "fourierspacing": 0.12,
        "ewald_rtol": 1e-5,
        "why": "Switches the reciprocal solver from PME to Ewald while keeping the same authoritative short-range baseline.",
        "intended_path_change": "pme_vs_ewald_downstream_trace",
    },
]
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            parsed: dict[str, object] = {
                "step": int(row["step"]),
                "stage": row["stage"],
                "compute_longrange_nonbonded_forces": int(row["compute_longrange_nonbonded_forces"]),
                "compute_energy": int(row["compute_energy"]),
                "compute_virial": int(row["compute_virial"]),
                "have_force_with_virial": int(row["have_force_with_virial"]),
                "buffers_share_storage": int(row["buffers_share_storage"]),
            }
...
def summarize_trace(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for stage in stages:
        ...
        summary["stages"][stage] = {
            ...
            "buffers_share_storage_any": any(int(row["buffers_share_storage"]) for row in stage_rows),
            "mean_coulomb_recip_term_kj_early": mean([float(row["coulomb_recip_term_kj"]) for row in early_rows]),
            "mean_potential_energy_kj_early": mean([float(row["potential_energy_kj"]) for row in early_rows]),
            "mean_force_with_virial_l2_early": mean([float(row["force_with_virial_l2"]) for row in early_rows]),
            "mean_final_force_l2_early": mean([float(row["final_force_l2"]) for row in early_rows]),
            "mean_direct_virial_trace_early": mean([float(row["direct_virial_trace"]) for row in early_rows]),
            "mean_vir_force_trace_early": mean([float(row["vir_force_trace"]) for row in early_rows]),
            "mean_coulomb_recip_term_kj_full": mean([float(row["coulomb_recip_term_kj"]) for row in stage_rows]),
            "mean_potential_energy_kj_full": mean([float(row["potential_energy_kj"]) for row in stage_rows]),
            "mean_force_with_virial_l2_full": mean([float(row["force_with_virial_l2"]) for row in stage_rows]),
            "mean_final_force_l2_full": mean([float(row["final_force_l2"]) for row in stage_rows]),
            "mean_direct_virial_trace_full": mean([float(row["direct_virial_trace"]) for row in stage_rows]),
            "mean_vir_force_trace_full": mean([float(row["vir_force_trace"]) for row in stage_rows]),
        }
    return summary
...
def run_case(run: dict[str, object], commands: list[str]) -> dict[str, object]:
    ...
    env = {"GMX_TP18D_TRACE_FILE": str(trace_output)}
    ...
    if not trace_output.exists():
        raise RuntimeError(f"{run_id} did not produce TP1.8d trace output")
...
def build_downstream_path_map() -> dict[str, object]:
    return {
        "milestone": "TP1.8d",
        ...
        "paths": [
            {
                ...
                "role": "Producer boundary for long-range Coulomb force, reciprocal energy, and direct virial outputs.",
                "status": "active_upstream_reference",
                "why": "TP1.8c already traced the PME-versus-Ewald split at this boundary.",
            },
            {
                "file": "src/gromacs/mdlib/sim_util.cpp",
                "function": "do_force",
                "role": "Caller-side handoff that passes ForceWithVirial and energy terms from long-range calculation into downstream post-processing.",
                "status": "active_and_traced",
                "why": "TP1.8d records after_longrange and after_accumulate_energy snapshots here.",
            },
            {
                "file": "src/gromacs/mdlib/sim_util.cpp",
                "function": "postProcessForces",
                "role": "Immediate consumer that sums ForceWithVirial.force_ into the final force buffer and adds direct virial contributions to vir_force.",
                "status": "active_and_traced_at_caller_boundary",
                "why": "TP1.8d records before_postprocess and after_postprocess snapshots around this consumer.",
            },
            {
                "file": "src/gromacs/mdlib/enerdata_utils.cpp",
                "function": "accumulatePotentialEnergies",
                "role": "Finalizes potential-energy bookkeeping from per-term energy contributions.",
                "status": "active_and_traced_at_caller_boundary",
                "why": "TP1.8d records after_accumulate_energy snapshots to see whether the Coulomb reciprocal term survives into final potential bookkeeping.",
            },
            {
                "file": "src/gromacs/mdtypes/forceoutput.h",
                "function": "ForceWithVirial::addVirialContribution / getVirial",
                "role": "Storage and accumulation buffer for direct virial contributions consumed by postProcessForces.",
                "status": "active",
                "why": "This is the object whose contents are transferred downstream into vir_force.",
            },
        ],
...
def main() -> None:
    ...
    recommendation = {
        ...
        "final_classification": "still_unresolved",
        "tp1_4_lj_pme_path_status": "inactive",
        "exact_next_step_recommendation": "Keep the authoritative safe baseline fixed and trace one level deeper into the post-selection Coulomb force/virial consumers only if the TP1.8d downstream aggregates still leave ambiguity; do not patch production logic yet.",
    }
    ...
    run_matrix = {
        ...
        "rationale_for_20ps_window": "TP1.8c already used a 20 ps authoritative trace window with runaway onset at 0.2 ps, so TP1.8d keeps the same window while tracing downstream consumers.",
    }

    write_text(RESULTS_DIR / "run_matrix.json", json.dumps(run_matrix, indent=2) + "\n")
    write_text(RESULTS_DIR / "downstream_path_map.json", json.dumps(build_downstream_path_map(), indent=2) + "\n")
    write_text(RESULTS_DIR / "downstream_trace_summary.json", json.dumps(summary, indent=2) + "\n")
    write_text(RESULTS_DIR / "tp1_8d_recommendation.json", json.dumps(recommendation, indent=2) + "\n")
