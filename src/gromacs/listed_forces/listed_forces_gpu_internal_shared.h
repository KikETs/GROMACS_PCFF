                                                       ...
                                                       const int                     localId)
{
    ...
    float rinvRepulsion =
            (repulsionPower == 12.0F) ? rinv6 * rinv6 : powf(rinv, repulsionPower);

    /* Calculate the Coulomb force * r */
    float velec = scale_factor * qq * rinv;

    /* Calculate the LJ force * r and add it to the Coulomb part */
    float fr = repulsionPower * c12 * rinvRepulsion - 6.0F * c6 * rinv6 + velec;
    ...
    if constexpr (calcEner)
    {
        *vtot_loc += c12 * rinvRepulsion - c6 * rinv6;
        *vtotElec_loc += velec;
    }
}
